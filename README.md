...
백엔드 구현 코드 기준으로는 전체 코드량의 약 **60%**가 Codex를 통해 초안 생성 또는 구현 보조를 받은 것으로 추정했습니다. FastAPI 라우터 구성, SQLAlchemy 모델 정의, 카카오 로그인 이후 자체 JWT를 발급하는 인증 흐름, 포인트 계산 API, 상점 구매/장착 API, Gemini API를 이용한 PDF 기반 문장 생성 로직 작성 과정에서 Codex를 주로 활용했습니다.

백엔드 개발 초기에는 Superpowers 플러그인을 사용해 문답 형식으로 요구사항을 더 세밀하게 구체화했습니다. 이를 통해 MVP에서 구현할 기능과 제외할 기능, ERD 범위, API 동작 방식, PDF 처리 방식, 포인트 정책을 정리했습니다. 이후 구체화된 명세를 바탕으로 Codex의 `/goal` 기능을 사용해 구현 작업을 랄프 루프 형태로 반복 실행하며 코드 초안 작성, 오류 수정, 구조 정리를 진행했습니다.

다만 핵심 설계 결정과 최종 검토는 사람이 직접 수행했습니다. SQLite는 파일 기반 데이터베이스이므로 실제 데이터 조회와 확인은 DBeaver GUI를 사용해 직접 진행했고, AWS 대시보드에서 이루어진 EC2, S3, 도메인, 환경변수, 배포 관련 인프라 설정도 수작업으로 진행했습니다. 또한 카카오 로그인 Redirect URI 설정, CORS 허용 도메인 수정, 프론트엔드 배포 주소와 백엔드 API 주소 연결, 실제 배포 환경에서의 로그인/API 호출 확인처럼 프론트엔드와 맞춰야 하는 작업은 라이브 세팅 방식으로 직접 조정했습니다.

| 구분 | 비율 | 설명 |
| --- | ---: | --- |
| Claude 기반 프론트엔드 구현 보조 | 80% | React 컴포넌트, 상태 관리, API 연동, UI 레이아웃, 반복 코드 초안 생성 |
| Codex 기반 백엔드 구현 보조 | 60% | FastAPI 라우터, SQLAlchemy 모델, 인증/JWT, 포인트 계산, 상점 API, Gemini 연동 코드 초안 생성 및 오류 수정 |
| Superpowers 기반 백엔드 기획 구체화 | - | 문답 형식으로 MVP 범위, ERD, API 정책, PDF 처리 방식, 포인트 정책을 구체화 |
| 사람이 직접 수행한 작업 | - | 핵심 요구사항 결정, 최종 코드 검토, DBeaver 기반 SQLite 데이터 확인, AWS 대시보드 인프라 설정, 카카오 로그인/CORS/배포 주소 등 라이브 연동 세팅, 최종 통합 |
